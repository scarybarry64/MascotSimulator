public static class CollisionTags
{
    // Characters
    public const string PLAYER = "Player";

    // Items
    public const string ITEM_PIZZA = "PizzaItem"; // replenishes health
    public const string ITEM_EMPLOYEE_CARD_UPGRADE = "EmployeeCardUpgradeItem"; // opens door
    public const string ITEM_TISSUES = "TissuesItem"; // cleans/blocks boogers
    public const string ITEM_CANDY = "CandyItem"; // distracts candy girl
    public const string ITEM_SODA = "SodaItem"; // gives temp speed buff

    // Triggers
    public const string DOOR_EXIT = "ExitDoor";
    public const string PRINCESS_COMMAND_ZONE = "PrincessCommandZone";
}
